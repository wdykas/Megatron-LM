...
class TestSelectiveStateUpdateInputPurity:
    ...
    def test_inputs_not_mutated_without_intermediates(self, seq_len):
        ...
        torch.manual_seed(0)
        device = "cuda"
        batch, nheads, headdim, dstate, ngroups = 4, 8, 64, 64, 2
        x = torch.randn(batch, seq_len, nheads, headdim, device=device)
        dt = (
            torch.randn(batch, seq_len, nheads, device=device)
            .unsqueeze(-1)
            .expand(batch, seq_len, nheads, headdim)
        )
        a = (
        ...
        b = torch.randn(batch, seq_len, ngroups, dstate, device=device)
        c = torch.randn(batch, seq_len, ngroups, dstate, device=device)
        ...
        state = torch.randn(batch, nheads, headdim, dstate, device=device)
        inputs = {"x": x, "dt": dt, "A": a, "B": b, "C": c, "D": d, "dt_bias": dt_bias}
        ...
        selective_state_update(
            ...
            dt_bias=dt_bias,
            dt_softplus=True,
            state_batch_indices=torch.arange(batch, dtype=torch.int64, device=device),
        )
