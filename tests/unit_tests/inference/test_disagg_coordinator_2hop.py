...
def _coord():
    ...
    c._req_meta = {}
    # Push engines (is_pull=False) -> unthrottled; REGISTER_ROLE records an entry
    # for every engine, so the map is dense (matches _route_submit_disagg).
    c._engine_is_pull = {b"p0": False, b"d0": False}
    c._disagg_prefill_of = {}
    c._disagg_outstanding = {}
    c._disagg_submit_queue = {}
    c._disagg_max_outstanding = 32
