            ...
            with nvtx_range("collect-rollouts"):
                if rank == 0:
                    ...
                    for _prompt_idx in range(n_prompts):
                        ...
                        # NOTE: compactor training data AND trajectory-dir saving both
                        # happen in build_compactor_trajectories (compaction/learned/
                        # online.py): compactor_raw_sequences is populated on ALL ranks
                        # below from the broadcast `rollouts`, then a collective forward
                        # replays each sequence so every rank captures its own TP-local
                        # KV slice. (The paged cache is drained by the time a rank-0
                        # hook could read it, so capture must go through that replay.)
