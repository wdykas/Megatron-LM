...
class SpeculativeRoute:
    """A bundle of N alternative :class:`Route` objects, exactly one
    of which will win.

    The producer submits one normal request per branch (each with its
    own ``server_request_id`` but the same prompt + sampling params),
    runs them in parallel for the first ``decision_layer`` layers,
    then picks a winner by ``policy`` and cancels the rest via
    :meth:`InferenceClient.cancel_branch`.

    Attributes:
        branches: The N alternative routes. Order is significant — the
            "default winner" used by the simple ``"first"`` policy is
            ``branches[0]``. All branches MUST share the same
            ``entry_shard`` and ``exit_shard`` so the coord's reply
            routing doesn't change per branch.
        decision_layer: Global layer index at which a winner is
            chosen. Branches whose route has progressed past this
            layer at decision time are eligible (the rest get
            cancelled regardless of policy verdict). Tunable per
            request — higher ``decision_layer`` increases the
            "exploration budget" at the cost of duplicating more
            compute / activation traffic.
        policy: How to pick the winner once decision_layer is reached
            across all branches. Supported values:

            - ``"first"`` — whichever branch reaches the decision
              layer first (selected by the producer based on its own
              ``RESPONSE_ACK`` tracking).
            - ``"min_load"`` — the branch whose downstream shards have
              the fewest in-flight requests at decision time. Requires
              the producer to track shard loads (already available in
              ``MegatronLocalMulti._in_flight``).
    """

    branches: Tuple[Route, ...]
    decision_layer: int
    policy: str = "first"

    def __post_init__(self) -> None:
        assert len(self.branches) >= 2, (
            "SpeculativeRoute needs at least 2 branches; a single "
            "branch is just a regular Route."
        )
        entries = {b.entry_shard for b in self.branches}
        exits = {b.exit_shard for b in self.branches}
        assert len(entries) == 1, (
            f"SpeculativeRoute branches must share entry_shard; "
            f"got {entries}"
        )
        assert len(exits) == 1, (
            f"SpeculativeRoute branches must share exit_shard; "
            f"got {exits}"
        )
        assert self.policy in ("first", "min_load"), (
            f"SpeculativeRoute: unsupported policy={self.policy!r}; "
            f"expected 'first' or 'min_load'."
        )

    @property
    def entry_shard(self) -> int:
        return self.branches[0].entry_shard

    @property
    def exit_shard(self) -> int:
        return self.branches[0].exit_shard

    def participating_shards(self) -> List[int]:
        """Union of shards visited by every branch — useful when the
        producer wants to know "what's the maximum footprint of this
        speculative request before we decide a winner?" """
        out: set = set()
...
def make_speculative_route(
    branches: Tuple[Route, ...],
    decision_layer: int,
    *,
    policy: str = "first",
) -> SpeculativeRoute:
    """Convenience constructor for :class:`SpeculativeRoute`. Pre-validates
    each branch has at least one hop and the decision_layer is within
    every branch's layer span (otherwise some branches would never
    reach the decision boundary and the policy can't fire)."""
    assert branches, "make_speculative_route needs at least one branch"
    for i, br in enumerate(branches):
        all_layers = [li for h in br.hops for li in h.layer_indices]
        assert decision_layer in all_layers, (
            f"make_speculative_route: branch {i} doesn't include "
            f"decision_layer={decision_layer} in its layer span "
            f"({all_layers}). All branches must reach the boundary."
        )
