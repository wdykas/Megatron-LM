...
def can_admit_prefilled_decode(context, input_token_count: int) -> bool:
    ...
    if context.active_token_count + input_token_count > context.max_tokens:
        return False
    if context.is_hybrid_model and context.mamba_metadata.mamba_state_free_slot_count < 1:
        return False
    return True
...
def admit_prefilled_decode(
    ...
    ssm_state_block_id: int | None = None,
) -> None:
    ...
    if context.is_hybrid_model:
        if ssm_state_block_id is None:
            raise ValueError("A hybrid decode request requires an exact imported SSM state")
        ssm_state_idx = context.mamba_metadata.allocate_slot()
        if ssm_state_idx is None:
            raise RuntimeError("No live SSM state slot available for imported decode state")
    ...
    if context.is_hybrid_model:
        ...
        context._pending_mamba_restores.append((current_id, ssm_state_block_id, ssm_state_idx))
