...
class InferenceStateHandoffMixin:
        ...
        try:
            ...
            if self._ssm_transfer_agents:
                ...
                flat_blocks = [
                    ...
                    for block in candidate_blocks_by_request[request.request_id]
                ]
                flat_slots = ssm_cache.get_slots(flat_blocks)
                slot_offset = 0
                for request, _ in handoffs:
                    block_count = len(candidate_blocks_by_request[request.request_id])
                    request_slots = flat_slots[slot_offset : slot_offset + block_count]
                    slot_offset += block_count
                    position_to_slot = {
                        position: int(slot)
                        for position, slot in enumerate(request_slots)
                        if slot >= 0
                    }
                    local_position_to_slot[request.request_id] = position_to_slot
            ...
            for request, _ in handoffs:
                ...
                position_to_slot = local_position_to_slot.get(request.request_id, {})
                ...
                if self._ssm_transfer_agents and final_position not in position_to_slot:
                    raise RuntimeError(
                ...
                if self._ssm_transfer_agents:
                    ...
                    local_ssm_slots = [position_to_slot[position] for position in positions]
                    stage_state_metas = [
    ...
    def add_request_with_kv_handoff(
        ...
        if not started:
            ...
            if capacity_error is None:
                ...
                logging.debug(
                    "DISAGG_DECODE_CAPACITY_QUEUE request_id=%d required=%d "
                    "available=%d queued=%d",
                    request_id,
    ...
    ) -> tuple[bool, MambaSlotCapacityError | None]:
    ...
    def _validate_decode_ready_handoff(self, pending: PendingKvImport) -> None:
        ...
        if self.context.is_hybrid_model:
            ...
            if pending.ssm.positions != [final_block_position]:
                raise RuntimeError(
    ...
    def _finalize_kv_handoff_import(self, pending: PendingKvImport) -> None:
        ...
        if pending.request_id not in self.waiting_request_ids:
            ...
            ssm_state_block_id = pending.ssm.target_blocks[0] if pending.ssm is not None else None
            admit_prefilled_decode(
                self.context,
                request,
                local_blocks,
    ...
    def _reserve_ssm_handoff_import(
        ...
        if any(
            pos < 0
            or pos >= len(local_blocks)
            or (pos >= len(hashes) and positions != [len(local_blocks) - 1])
    ...
    def _complete_ssm_handoff_import(
        ...
        hashed_states = [
            (block_id, hashes[position])
            for block_id, position in zip(pending.target_blocks, pending.positions)
            if position < len(hashes)
        ]
        if hashed_states:
            block_ids, block_hashes = zip(*hashed_states)
            ssm_cache.register_block_hashes_batch(list(block_ids), list(block_hashes))
        logging.debug(
