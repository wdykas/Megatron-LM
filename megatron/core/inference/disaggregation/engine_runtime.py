...
The runtime branches only on ``self.is_pull`` (one-sided NIXL vs two-sided
...
def kv_layouts(dicts):
    """KVShardLayout list from wire dicts, stripping the optional hybrid
    ``mamba`` sub-dict (a separate layout)."""
...
class DisaggEngineRuntime:
    ...
    def get_backend(self):
        ...
        one-sided pull). The runtime branches on ``self.is_pull`` -- everything
        else is transport-agnostic.

        Only push (two-sided) backends reach this lazy path. Pull backends are
        built and their KV (+ Mamba snapshot) buffers registered eagerly in
        ``__init__`` (before the first prefill), so ``self.backend`` is already
        set for them and a peer can READ entries by block/slot index without
    ...
    def register_pull_regions(self, backend):
        ...
        KV blocks on axis 2 of ``(2, L, blocks, BS, H, HD)``, Mamba snapshot
        slots on axis 1 of ``(layers, slots, *state)``. Also sets
        ...
        - Mamba snapshots (hybrid): the block-boundary states, registered by
          reference on both sides -- the snapshot pool isn't reset mid-rollout
          and the KV pin keeps a published request's snapshots alive. The live
          end-state is never transferred (it is only correct when zero prompt
          tokens re-run, which admission never does).
        """
        ctx = self.context
    ...
    def publish_push_handoff(self, request_id):
        """(prefill, push) The PREFILL_DONE hand-off for a push transport: only
        the Mamba snapshot hashes (the decode can't derive the snapshot count
        header-free; everything else it rebuilds from the prompt). Reads the
        staged export without consuming it -- SEND_KV drains it later. Returns
    ...
    def send_kv(self, request_id, dst_layout_dicts):
        ...
        # The decode posted its matched receives when RECV_KV arrived, so a
        # missing staged export is a protocol violation -- skipping the send
        # would leave the decode blocked in an unmatched NCCL recv forever.
    ...
    def recv_kv(self, request_id, src_layout_dicts, prompt, sampling_params, handoff):
        ...
        ``handoff`` carries only the snapshot hashes. One-sided backends (NIXL)
        instead allocate destination blocks and issue a one-sided READ of the
        prefill's blocks into them; ``handoff`` carries the per-rank region meta
        + source block ids, relayed by the coordinator. Both paths yield an
        object with a ``finish(engine)`` that commits, so the rest is symmetric."""
    ...
    def registration_message(self):
        ...
        prefill's pin window) only for pull instances."""
    ...
    def send_prefill_done(self, records_to_send):
        ...
        handoff. Push backends attach only the snapshot hashes; the KV ships
        later on SEND_KV."""
    ...
    def release_pinned(self, request_id):
        ...
        request's pinned KV blocks (which also frees their boundary snapshots
        for eviction)."""
