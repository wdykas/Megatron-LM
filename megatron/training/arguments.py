...
def _add_inference_args(parser):
    ...
    group.add_argument('--inference-replicate-requests', action=argparse.BooleanOptionalAction,
                       ...
                       help='If set, the data-parallel inference coordinator broadcasts each '
                            ...
                            'the skip-AG MoE inference path. Pairs with '
    ...
    group.add_argument('--inference-partitioned-state', action=argparse.BooleanOptionalAction,
                       required=False, default=False,
                       help='If set, run the skip-AG MoE path with per-rank request ownership '
                            'instead of request replication. Each request lives on exactly one '
                            'EP-rank owner; mamba state is per-rank; the global view used by '
                            'skip-AG MoE is produced by an NVLS all-gather of the owners\' mamba '
                            'outputs. Mutually exclusive with --inference-replicate-requests.')
